...
7. 타이틀 화면 다음에는 게임 조작법과 한국어로 설명하는 화면을 보여주세요.
8. font가 필요한 경우 기본 font를 사용해야 하며 에셋에 새로운 font를 포함시키지 마세요.

응답은 반드시 아래의 커스텀 형식에 맞춰주세요. 어떠한 설명이나 마크다운 구문(```)도 태그 외부에 넣지 마세요. 태그 내부에는 아래에 명시된 지침문은 제거하고 생성한 내용만 넣으세요.

###CODE_START###
game.ts에 저장될 TypeScript 코드 전체를 이곳에 넣으세요. 만약 변경된 내용이 없다면 비워두세요.
###CODE_END###
